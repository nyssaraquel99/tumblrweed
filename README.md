...
I wrote **tumblrweed** and **nuke** because I wanted to refresh my blog, but Tumblr doesn't natively have a way to easily delete all posts. Even the Mass Post Editor requires you to select each post to delete one by one. I had nearly 3000 posts on my 14 year old blog, so that simply wasn't going to happen. But I didn't want to give up my url or make a new blog. So over the course of four days, I riffled through Tumblr's API docs and put this script together to automate the process. Would it have been faster to suffer through Mass Post Editor and delete my posts manually? Probably. Could I have use the pytumblr client library instead of writing my own code? Sure. But now I have these scripts at hand in case of my next ego death. And, thanks to this repo, so do you!   
- - - - - - - - - - - -   
Notes:   
...
The client tokens are retrieved after successfully registering your application. The OAuth1 tokens are retrieved from running **app** and opening the Redirect URI in your browser of choice.   
2. **tumblrweed** is optimized for creating simple text posts for testing purposes, and for deleting posts. You can use *get_info* to verify your API keys are working, and *get_follower_count* to verify your OAuth1 keys are working. If you want to do anything more than creating test posts and deleting posts, I highly recommend looking at Tumblr's client libraries, as listed on their API docs. Otherwise, you can also use *get_res* to call the API, referencing the API docs for endpoints and parameters to include.   
...
4. **tumblrweed** was last updated May 3, 2026.   
